...
## Development of Odin: A Personal Finance Management System Using Random Forest, LSTM, and Isolation Forest
...
> **Cross‑reference:** Detailed mobile constraints are consolidated in Topic 9.A.III. This sub‑subtopic should reference that discussion and apply it specifically to forecasting.
...
### 7.E Onboarding and Cold-Start for Budget Recommendation [/budget-onboarding]

> **Note:** This section covers the data collection, feature extraction, and initial calibration that feed into the budget recommendation module at system launch. It bridges the gap between user registration and the first personalized budget output.

#### 7.E.I Questionnaire design for initial financial profile extraction [/onboarding-questionnaire]
> *Scholar query: How should an onboarding questionnaire be designed to extract sufficient information for initial financial behavioral profile classification and budget recommendation in a personal finance system? I am looking for research on questionnaire length, item selection, response format, and the tradeoff between comprehensive data collection and user dropout during onboarding — particularly for financial applications serving young adults with variable income.*

#### 7.E.II Feature engineering from onboarding responses [/onboarding-feature-engineering]
> *Scholar query: What features can be reliably extracted from onboarding questionnaire responses to initialize a financial behavioral profile classifier and budget recommendation engine? I am looking for studies that map self-reported financial behaviors (income type, employment status, obligation weight, savings habits) to quantitative features suitable for ML classification, and that evaluate which questionnaire items carry the most predictive power for downstream profile assignment.*

#### 7.E.III Progressive refinement from onboarding defaults to behavioral personalization [/onboarding-progressive-refinement]
> *Scholar query: How should a personal finance system transition from initial onboarding-derived defaults to behaviorally personalized budget recommendations as transaction data accumulates? I am looking for research on the convergence timeline from cold-start to personalized recommendations, the data volume thresholds at which personalization becomes reliable, and strategies for maintaining user trust during the transition period when recommendations are still based on limited data.*
...
#### 9.A.III Technical constraints for ML models on mobile: compute, connectivity, screen size, and battery life [/mobile-technical-constraints]
...
### 9.C Offline-First Architecture [/offline-first]

> **Note:** This section covers the architectural patterns for building a mobile financial application that functions fully without network connectivity and synchronizes transparently when connectivity returns. It extends Topic 9.B.III with deeper coverage of local-first database design, sync engine patterns, and offline-capable ML inference.

#### 9.C.I SQLite local-first databases: schema design, migration strategies, and conflict resolution [/offline-sqlite]
> *Scholar query: What architectural patterns exist for designing SQLite-based local-first databases in mobile financial applications — including schema design for offline reads and writes, migration strategies that preserve local data across app updates, and conflict resolution when local and remote states diverge? I am looking for system design literature that addresses the specific challenges of financial data integrity in offline-first architectures, where lost transactions or sync conflicts can have direct monetary consequences.*

#### 9.C.II Sync engines: cursor-based push/pull, queue management, and idempotency [/offline-sync-engines]
> *Scholar query: What synchronization engine architectures have been implemented for offline-capable mobile applications that must reconcile locally queued operations with a remote server? I am looking for research on cursor-based push/pull sync protocols, operation queue management (ordering, deduplication, retry), and idempotency guarantees that prevent duplicate financial transactions during sync — particularly in environments with intermittent or unreliable connectivity.*

#### 9.C.III Offline-capable ML inference: what works offline versus what requires connectivity [/offline-ml-inference]
> *Scholar query: Which machine learning components of a personal finance management system can feasibly run on-device without network connectivity, and which require server-side computation? I am looking for research that evaluates the feasibility of running classification, forecasting, and anomaly detection models locally on mobile hardware — considering model size, inference latency, battery impact, and the accuracy tradeoffs of smaller on-device models versus larger server-side models.*
...
### 10.C Consent Management and Data Portability [/consent-portability]

> **Note:** This section covers the legal and technical requirements for user consent, data export, and data deletion in personal finance applications, with specific attention to the Philippine regulatory context.

#### 10.C.I Consent recording and granularity in financial applications [/consent-recording]
> *Scholar query: How should personal finance applications implement consent recording and management — specifically the granularity of consent (e.g., separate consents for personalization, model training, research use, notifications), the technical mechanisms for recording and retrieving consent state, and the user experience of consent management? I am looking for research on consent design patterns that balance regulatory compliance with usability, particularly for financial applications where consent granularity affects what ML features can be offered.*

#### 10.C.II Data export, portability, and deletion under RA 10173 [/data-portability-ph]
> *Scholar query: What are the technical requirements and implementation patterns for data export, portability, and account deletion in personal finance applications operating under the Philippine Data Privacy Act of 2012 (RA 10173)? I am looking for research or compliance literature that describes how to implement data subject access requests, right-to-erasure workflows, and portable data formats for financial transaction records — including the challenge of deleting data that has already been incorporated into trained ML models.*

#### 10.C.III Privacy-preserving ML: model training without exposing raw transaction data [/privacy-preserving-ml]
> *Scholar query: What techniques exist for training machine learning models on personal financial data without exposing individual users' raw transaction records? I am looking for research on federated learning, differential privacy, secure multi-party computation, and synthetic data generation as approaches to privacy-preserving financial ML — with attention to the accuracy-privacy tradeoff and the feasibility of these techniques on mobile-first personal finance systems.*
...
## 13. Savings and Debt Management Algorithms [/savings-debt]

### 13.A Savings Goal Management in PFMS [/savings-domain]

#### 13.A.I Deterministic goal tracking vs. investment tracking (exclusion justification) [/goal-tracking-exclusion]
> *Scholar query: How is deterministic savings goal tracking — where the accumulation amount is known and only contributions matter — distinguished from investment tracking with variable returns in personal finance systems, and what literature supports the exclusion of investment tracking from a system focused on budgeting and saving?*

#### 13.A.II Contribution allocation strategies: priority‑based, smallest‑balance (snowball), and highest‑priority (avalanche) [/savings-allocation-strategies]
> *Scholar query: What evidence exists that users prefer different savings contribution allocation strategies (e.g., prioritising one goal over others vs. paying off the smallest goal first) and how do these strategies affect goal completion rates? I am looking for behavioral or system design studies that compare priority‑based allocation, snowball (smallest balance first), and avalanche (highest priority first) for savings goals.*

#### 13.A.III Surplus handling in savings (as distinct from budget surplus) [/savings-surplus]
> *Scholar query: How do personal finance systems treat unexpected surplus funds — such as budget underspend or windfall income — in relation to savings goals, and what literature discusses the behavioral effects of automatically directing surplus to savings versus allowing user discretion?*

---

### 13.B Debt Management in PFMS [/debt-domain]

#### 13.B.I Simple vs. compound interest modeling (exclusion justification) [/debt-interest-models]
> *Scholar query: What literature distinguishes between simple and compound interest modeling in personal debt management systems, and what justifications exist for excluding compound interest (e.g., credit card revolving debt) from a thesis‑scope system focused on simple‑interest loans?*

#### 13.B.II Debt payoff strategies: avalanche vs. snowball – behavioral evidence [/debt-payoff-strategies]
> *Scholar query: What comparative studies evaluate the effectiveness of avalanche vs. snowball debt payoff strategies in terms of total interest paid vs. psychological motivation and completion rates? I am looking for behavioral finance or system evaluation research that reports which strategy leads to higher debt reduction and which leads to higher user persistence.*
...
## 14. ML Pipeline and Infrastructure [/ml-pipeline]

> **Note:** This topic covers the machine learning infrastructure required to train, serve, and maintain the models that power the intelligent modules described in Topics 5–8. It addresses the engineering concerns that arise when ML models move from research notebooks to production systems serving real users.

### 14.A Synthetic Data Generation for Financial ML [/synthetic-data]

> **Note:** This section covers the generation of synthetic financial data for model training and evaluation when real labeled data is unavailable or insufficient. It addresses persona generation, transaction simulation, and validation of synthetic distributions against real-world data.

#### 14.A.I Persona generation from survey data: archetypes, feature distributions, and calibration [/persona-generation]
> *Scholar query: How have researchers generated synthetic user personas from household survey data for training financial ML models — including the definition of behavioral archetypes, the specification of feature distributions for each archetype, and the calibration of synthetic personas against real survey statistics? I am looking for studies that describe methodologies for creating diverse, realistic synthetic populations from instruments such as the Family Income and Expenditure Survey, and that evaluate whether synthetic personas produce ML models that generalize to real users.*

#### 14.A.II Transaction simulation: temporal patterns, anomaly injection, and cultural spending injection [/transaction-simulation]
> *Scholar query: What methodologies exist for simulating realistic financial transaction histories from synthetic personas — including the modeling of temporal spending patterns (payday effects, seasonal cycles, day-of-week effects), the injection of known anomalies for supervised or semi-supervised training, and the incorporation of culturally specific spending events (holiday spikes, community contributions, irregular income)? I am looking for research that evaluates the realism of simulated transaction data and its effectiveness as training data for spending forecasting and anomaly detection models.*

#### 14.A.III Validation of synthetic data against real-world distributions [/synthetic-validation]
> *Scholar query: What evaluation methodologies are appropriate for assessing the quality and realism of synthetic financial data generated for ML training? I am looking for research on distributional comparison tests, downstream model performance comparisons (synthetic-trained vs. real-trained), and domain expert validation approaches — with attention to the specific challenges of validating financial data where privacy constraints prevent direct comparison with real records.*

---

### 14.B ML Model Serving and Deployment [/ml-serving]

> **Note:** This section covers the infrastructure for serving trained ML models to end users via API endpoints, with attention to latency, versioning, and scalability constraints imposed by mobile-first deployment.

#### 14.B.I FastAPI microservice architecture for model inference [/fastapi-serving]
> *Scholar query: What architectural patterns are appropriate for serving ML models via REST API microservices in personal finance applications — specifically using frameworks such as FastAPI? I am looking for research or system design literature that discusses request/response design for model inference endpoints, model loading and caching strategies, health monitoring, and the tradeoffs between monolithic and microservice architectures for serving multiple ML modules (classification, forecasting, anomaly detection).*

#### 14.B.II Model versioning, rollback, and A/B testing [/model-versioning]
> *Scholar query: What strategies exist for versioning, rolling back, and A/B testing ML models in production personal finance systems? I am looking for research on model registry design, canary deployments for gradual rollout, automated rollback triggers based on inference quality degradation, and A/B testing frameworks that measure the behavioral impact of model changes on user financial outcomes.*

#### 14.B.III Latency and throughput constraints for mobile-first inference [/serving-latency]
> *Scholar query: What latency and throughput requirements apply to ML model serving in mobile-first personal finance applications, and how do these constraints shape the design of inference pipelines? I am looking for research on acceptable inference response times for user-facing financial features (budget recommendations, anomaly alerts, spending forecasts), the impact of network latency on mobile user experience, and strategies for meeting latency SLAs including model optimization, caching, and precomputation.*

---

### 14.C On-Device vs. Server Inference [/inference-architecture]

> **Note:** This section covers the architectural decision of where to run ML inference — on the user's mobile device, on a remote server, or in a hybrid configuration — and the tradeoffs each approach entails.

#### 14.C.I Tradeoffs between on-device ML and cloud-based inference for PFMS [/on-device-vs-cloud]
> *Scholar query: What are the tradeoffs between on-device and cloud-based ML inference in personal finance management systems — considering accuracy (model size vs. capability), latency (local computation vs. network round-trip), privacy (data never leaves device vs. data transmitted to server), offline capability (works without connectivity vs. requires network), and cost (device battery and compute vs. server hosting)? I am looking for comparative analyses that evaluate these dimensions in the context of financial applications serving users with intermittent connectivity and varying device capabilities.*

#### 14.C.II Model compression, quantization, and mobile deployment strategies [/model-compression]
> *Scholar query: What model compression and quantization techniques are applicable to ML models deployed on mobile devices for personal finance inference — including knowledge distillation, weight pruning, int8 quantization, and TensorFlow Lite / ONNX Runtime conversion? I am looking for research that evaluates the accuracy-size-latency tradeoffs of these techniques for financial classification, forecasting, and anomaly detection models, and that reports practical deployment results on representative mobile hardware.*

#### 14.C.III Hybrid inference: offline fallback with server-side personalization [/hybrid-inference]
> *Scholar query: What architectures support hybrid inference in mobile personal finance systems — where a lightweight on-device model provides immediate offline predictions while a more capable server-side model provides personalized predictions when connectivity is available? I am looking for research on model selection strategies (when to use local vs. remote), prediction reconciliation (how to merge or replace local predictions with server predictions), and the user experience of inference quality that varies with connectivity status.*
