---
conversion_metadata:
  converted_at: "2026-07-20T15:28:21Z"
  converter_tool: "markitdown"
  converter_version: "0.1.6"
  source_pdf: "Chowdhury A. et al.pdf"
  source_pdf_sha256: "c97296e2a2aa7568f1c5ac79420683071e6834e09eb91cf78924fb4a8b2f72af"
  page_count: 27
  markdown_char_count: 247744
---
...
and evolution of demand forecasting methodologies specifically applied within the context of retail e-
commerce. The selected studies were rigorously scre ened and categorized based on methodological 
foundations, technological sophistication, and practical applications. Four major categories of forecasting 
models are examined: (1) traditional statistical approaches, such as AutoRegressive Integrated Moving 
Average (ARIMA), Seasonal ARIMA (SARIMA), and exponential smoothing models; (2) machine 
learning techniques, including decision trees, random forests, support vector regression, and ensemble 
methods; (3) hybrid frameworks that integrate statistical modeling  with machine learning or deep 
learning components; and (4) advanced deep learning architectures, particularly Long Short -Term 
Memory (LSTM) networks, Gated Recurrent Units (GRUs), Convolutional Neural Networks (CNNs), 
and transformer-based models. The fin dings reveal distinct performance advantages across model 
...
further advance this capability by capturing complex temporal dynamics, long-range dependencies, and 
multivariate input streams. These models are particularly effective for SKU -level forecasting, where 
product-specific demand patterns fluctuate frequently and require real -time recalibration.  Many 
reviewed studies adopted modular architectures that allow domain -specific tuning and facilitate 
deployment within enterprise resource planning (ERP) and inventory management systems. 
Keywords 
Demand Forecasting; Retail E-Commerce; Inventory Planning; Machine Learning; Forecasting Models. 
  
Citation 
Chowdhury, A. R., Paul, R., 
& Rozony, F. Z. (2025). A 
systematic review of demand 
forecasting models for retail e -
commerce enhancing accuracy 
in inventory and delivery 
planning. International Journal 
of Scientific Interdisciplinary 
Research, 6(1), 1–27. 
https://doi.org/10.63125/m
bbfw637 
...
Licensee  
IJSIR, Florida, USA 
This article is published as open 
access and may be freely shared, 
reproduced, or adapted for any 
lawful purpose, provided proper 
...
(Li & Xu, 2025 ). These foundational definitions set the stage for understandi ng how advanced 
forecasting methodologies are reshaping 
operational paradigms in digital commerce 
ecosystems (Li & Wang, 2020). 
The international sign ificance of demand 
...
2020, a fi gure that continues to rise as 
...
inefficiencies and consumer demand unpredictability. H ence, the international scope and 
...
demands nuanced and adaptive forecasting appr oaches. In this context, demand forecasting 
...
efficiently and cu stomers receive their orders on time. With the rise of same -day and next -day 
delivery expectations, especially in urban markets, logistics systems are under increasing pressure 
to respond dynamically to demand fluctuations. Effective demand forecasting sup ports route 
...
relied on classical statistical 
...
to digital commerce, these models leverage structured and unstructured dat a ranging from 
...
integration of various data sources, including transactional, behavior al, and environmental data, 
...
forecasting success. The study is dri ven by the need for a consolidated knowledge base that 
...
comprehensive synthesis of academic and applied research that explores v arious forecasting 
...
consumers researching on mobile and purchasing via d esktop, which introduces temporal and 
...
immediate and dramatic effect on online retail demand than on traditional retail due t o digital 
...
Among the most widely adopted are Mean Absolute Error (MAE), Roo t Mean Squared Error 
...
minimizing stockouts, op timizing fulfillment costs, or improving forecast stability across volatile 
...
decentralized. Accurate forecasts enable retailers to maint ain optimal stock levels, reducing both 
...
mere sales prediction. It s erves as 
...
and mov ing averages to model non -stationary time series data. When extended to account for 
seasonality, SARIMA becomes especially useful for modeling demand cycles such as weekly or 
monthly purchasing trends. Holt -Winters exponential smoothing, including both add itive and 
...
seasonal decomposition and preprocessing techniques to improve stability.  Despite their reli ance 
...
computational requirements and ease of deploym ent. Additionally, the viability of ARIMA in 
...
faced with sudden promotions, stockouts, or consumer behavior shifts, as no ted by Goltsos et al., 
(2018). Nevertheless, ARIMA, SARIMA, and Holt -Winters re main essential components in the 
...
observations, giving more importance to recent data while smoothing out historical nois e. Among 
...
exponential smoothing that enh anced its theoretical robustness and enabled probabilistic 
forecasting. 
...
promotional spikes, or irregular sales cycles, which are inc reasingly common in online 
...
Nevertheless, their speed, simplici ty, and relatively good performance on short -term horizons 
...
by inherent methodological assumptions. These models are typically based on li nearity, 
...
indicators. In contr ast, machine learning models can readily incorporate such multidimensional 
...
methods generally outper formed 
...
e-retail data for fashion and electronics, revealing that classical models pe rformed adequately in 
...
(2024) assessed multiple forecasting models for peri shable goods and found that exponential 
...
components within hybrid systems. Their continued evaluation in benc hmarking studies affirms 
...
to traditional time se ries models  (Khan, 2025 ; Md et al., 2025 ). XGBoost for short -term sales 
...
with short life cycles or erratic demand  (Jahan, 202 4; Islam et al., 2024 ). Policarpo et al. (2021) 
...
integration into ensemble frameworks or as benchmark models continues to e nhance the 
...
depend not only on the algorithmic architecture but also on the selection and transformatio n of 
...
2023). Promotion-related features, such as discount rates, marketing campaig ns, and flash sales, 
...
(2011) emphasized that model performance often hi nges more on the quality of features than the 
...
realities (Mahfuj e t al., 2022 ; Majharul et al., 2022 ). Au tomated feature engineering platforms, 
...
forecasting perf ormance in the e -commerce domain. Empirical evaluations of machine learning 
...
2018). These studies often reveal that machine learning models outperform cl assical techniques, 
...
Alibaba’s product dataset and reported superior accuracy from XGBoost and Lig htGBM models, 
...
across multiple categories.  Real-world evaluati ons also highlight issues of scalability and 
...
strong assumptions of data stationarity.  Several empirical studies have de monstrated the 
...
feature sets. DeepAR, an LSTM -based probabilisti c forecasting model, which has been widely 
...
While Convolution Neural Networks (CNNs) are traditionall y associated with image and spatial 
...
time-series data. Their parallelization capabilities and fast training cycles also make the m suitable 
...
and even LSTMs. CNNs’ ability to process multiple time -series inputs, including sales, marketin g 
...
(e.g., short -term peaks and valleys) that may correspond to marketing campaigns or stoc k 
...
Attention mechanisms themselves have also been embedded into hybrid LST M and CNN models 
...
knowledge distillation and quantization, have been used to reduce infere nce time in production 
...
support systems. Despite the complex ity, deep learning models continue to deliver value in high -
...
environments. A common hybrid design involves decomposing the time s eries into linear and 
...
computationally more in tensive but often 
...
interpretability and p redictive accuracy. 
...
of exploiting model di versity to capture 
...
is high. Gradie nt boosting methods like XGBoost and Light GBM have been extensively used to 
...
selection, hyper parameter tuning, and interpretability. Desp ite these barriers, ensemble learning 
...
fluctuations. These pipelines combine models in a sequential or modular architecture, where eac h 
...
particularly when dealing with SKU -level granularity and varied promotional s ensitivities. One 
...
promotional impacts) to forecast each componen t. Using DeepAR to model baseline sales while 
...
promotional bursts and LSTM layers for seasonal patterns yielded superior performance on h igh-
...
integrating multi-model pipelines lies in feature engineering, synchron ization of model outputs, 
...
is critical to determine their effectiveness, generalizability, and suitability for specific retai l 
...
(RMSE), Mean Absolute Percentage Error (MAPE), and Symmetric MA PE (sMAPE), along with 
...
in terms of MAE an d RMSE, particularly for volatile product categories. Deep learning ensembles 
with hybrid models and found that hybrid approaches yielded lower forecasting errors across 70% 
of SKUs in a multi -retailer dataset. Similarly, ARIMA -XGBoost hybrids achieved bet ter accuracy 
...
operational feasibility, making them well -suited to the multi faceted forecasting needs of retail e -
commerce. 
...
data, for instance, is particul arly relevant for forecasting sales of seasonal items such as apparel, 
...
durable goods on e -commerce platforms (Shen et al., 2021 ). Social media sig nals, such as likes, 
...
mentions and social news streams into 
...
behavior, which are otherwise diff icult to quantify through numerical transactions. Sentiment 
...
while negative feedback can predict demand decline. Similarly, Twitter sent iment to forecast 
...
example, sentiment extracted from blog posts p redicted box office performance more accurately 
...
sentiment data enhance demand forecastin g by capturing the "why" behind consumer behavior, 
making them an essential asset in the e-commerce forecasting toolkit. 
Figure 8: Incorporating External Signals in Demand 
Forecasting
...
poor forecasts leads to increased bullwhip effects, amplifying fluctuati ons in upstream inventory. 
...
stock cal culation, allowing firms to 
...
adopting demand-driven replenishment systems achiev ed higher inventory turnover and service 
...
supplier collaboration by generating demand forecasts that ca n be shared upstream, enhancing 
...
optimize last -mile rou ting. The value of integrating demand forecasts into delivery planning. 
...
availability and the ability to integrate forecasting outputs w ith transportation management 
systems.  
METHOD 
...
The revie w process began with a well -defined objective: to examine the breadth and depth of 
forecasting techniques employed in retail e-commerce and to assess their implications for inventory 
planning and delivery optimization. Based on this objective, a detailed p rotocol was designed to 
...
each st udy’s methodological rigor, 
...
or delivery), accuracy metrics reported, a nd any external/contextual variables considered (e.g., 
...
horizons. They were especially effective for replenishable products with p redictable consumption 
...
high citation frequency of these studies unde rscore the foundational role traditional models 
...
Models/Methods 
Main Contributions 
Traditional Statistical Models 21 2300 ARIMA, SARIMA, 
Holt-Winters 
Benchmarking, 
stability in 
seasonal demand, 
low cost 
Machine Learning Models 31 4800 Decision Trees, 
Random Forests, 
GBM, SVR, k-NN 
High adaptability, 
improved 
short/medium-
term accuracy 
Deep Learning Models 22 3900 LSTM, GRU, CNN, 
Transformers 
Captures dynamic 
& nonlinear 
patterns, high SKU 
volume 
Hybrid Forecasting Models 18 2500 ARIMA + ML/DL, 
Two-stage models 
Combines 
strengths of 
classical & AI 
methods, modular 
design 
External Data Integration 27 5000 Sentiment analysis, 
IoT, Clickstream, 
Social media 
Context-aware 
forecasting, real-
...
algorithms such as decision trees, random forests, gradient boosting mach ines, support vector 
...
models demonstrated high levels of adaptabi lity, outperforming traditional methods in 
...
the reviewed studies, which toge ther accounted for over 3,900 citations, indicating both their 
...
strengths of classical approaches like ARIMA for capturin g linear trends and seasonality, while 
...
demand signal and another corr ected deviations. These hybrid approaches showed notable 
...
The final and perhaps most transformative fi nding relates to the integration of external and 
...
for retail e-commerce. Despite their conceptual simplicity, thei r inclusion in 21 of the 72 reviewed
...
terms of computational efficiency and transparency, their solo application is often inadequat e for 
...
research to capture comp lex feature interactions and model multivariate dependencies. The 
...
in large -scale, multivariate datas ets (Razzaq & Shah, 2025 ). The inclusion of CNN models for 
...
interacting variables —hybrids consistently outperform s tandalone models. Thus, hybridization 
...
Moreover, the successful integration o f external data in deep learning architectures, such as 
...
standardization o f external data preprocessing, echoing concerns from Benleulmi et al. (2025). 
...
reduced stockout or overstock rates, as observed in multiple reviewed st udies, supports prior 
...
application to real -time, high -volume e -commerce environments. This review confirms that th e 
...
data integration and probabilistic modeling, has become an industry norm. The systema tic use of 
...
as Mercado Libre and Jumia offer additional perspec tives, highlighting the adaptability of 
...
based archi tectures demonstrate strong capabilities in capturing nonlinearities and long -term 
...
replenishment, and im proving delivery precision. Collectively, this review not only maps the 
...
[7]. Alfarisi, W. W., Haura, Z., Pratiwi, D. A., Putri, F. A., Sadewo, E., & Darmawan, G. (2024). Comparativ e 
...
[8]. Alghamdi, A. (2023). A hybrid method for big data analysis using fuzzy clustering, featur e selection and 
...
[9]. Ammar, B., Faria, J., Ishtiaque, A., & Noor Alam, S. (2024). A Systematic Li terature Review On AI -Enabled 
...
using deep layer recurrent neural n etworks (RNNs) with gated recurrent units (GRUs) and long short -term 
...
[16]. Bekkerman, R., Bilenko, M., & Lang ford, J. (2011). Scaling Up Machine Learning: Parallel and Distributed 
...
Renewable Hydrogen Production: A Revi ew. American Journal of Interdisciplinary Studies , 6(1), 76 -94. 
...
Experimental Investigation Of Vibration Isolation Characteristics Of Negative Stiffne ss System For Pipeline. 
Global Mainstream Journal of Innovation, Engineering & Emerging Technology , 2(01), 15 -32. 
https://doi.org/10.62304/jieet.v2i01.227  
[24]. Clottey, T., & Benton, W. C. (2014). Determ ining core acquisition quantities when products have long return 
lags. IIE Transactions, 46(9), 880-893. https://doi.org/10.1080/0740817x.2014.882531   
[25]. Cui, H., Rajagopalan, S., & Ward, A. R. (202 0). Predicting product return volume using machine learning 
...
[30]. Goedhart, J., Ha ijema, R., & Akkerman, R. (2023). Modelling the influence of returns for an omni -channel 
...
[41]. Islam, M. T., Islam, K. S., Hossain, A., & Khan, M. R. (2025). Reducing Operati onal Costs in U.S. Hospitals 
...
Behaviors And Safety Awareness In The Manufacturing Sector. American Journal of Advanced Technology an d 
...
[53]. Lara-Benítez, P., Carranza -García, M., Luna -Romera, J. M., & Riquelme, J. C. (2020). Tempora l convolutional 
...
commerce platforms. Intelligent Systems wit h Applications , 22, 200364 -200364. 
...
Energy Systems: Integrating Solar, Wind, And Biomass for Enhanced Sustainability And Performanc e. 
...
[65]. Md Masud, K. (2022). A Systematic Review Of Credit Risk Assess ment Models In Emerging Economies: A 
...
[76]. Razzaq, K., & Shah, M. (2025). Machine learning and deep learning paradigms: From techniques to pr actical 
...
[79]. Roksana, H. (2023). Au tomation In Manufacturing: A Systematic Review Of Advanced Time Management 
...
in Chronic Disease Management. Strategic Data Man agement and Innovation , 2(01), 160 -180. 
...
Enhanced Insights in Park inson's Disease Classification. 2024 International Conference on Intelligent 
...
[91]. Siddiqui, N. A. (2025). Optimizing Busi ness Decision -Making Through AI -Enhanced Business Intelligence 
...
Maturity Models. American Journal of Scholarly Research and Innovation , 2(02), 135 -165. 
https://doi.org/10.63125/pfdm9g02  
[93]. Sohel, A., Alam, M. A., Hossain, A., Mahmud, S., & Akter, S. (2022). Artificial Intellige nce In Predictive 
...
Systematic Review Of Data -Driven Approaches, Digital Twin s, And Self -Healing Grids. American Journal of 
...
[101]. Yin, B., Corradi, F., & Bohte, S. M. (2021). Accurate an d efficient time -domain classification with adaptive 
...
[105]. Zhu, Y., Li, J., He, J., Quan z, B., & Deshpande, A. A. (2018). IJCAI - A local algorithm for product return 
