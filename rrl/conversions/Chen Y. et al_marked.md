...
tree ensembles dominated linear models, and ex ploiting missingness patterns 
...
financial-ratio portrait that summarizes the characteristic liquid ity, leverage, 
...
auditors must consider whether to modify th e audit 
...
[15]. Subsequent w ork examined how audit report lag 
...
of ratios to construct interpretable discriminants. 
Altman’s Z -score [1] demonstrated that a linear 
combination of ratios can separate bankrup t and non -
...
profitability, weak l iquidity, and high leverage jointly 
signal distress. However, modern business 
...
to study algorithms and evaluation metrics that 
...
Surveys on imbalanced learning emphasize tha t the 
...
appealing because it ca n be applied as a preprocessing 
...
However, focal loss also introduces additional 
...
performance by emphasizing true negative rates. 
Precision–recall (PR) analysis provides a more 
...
PR curves are more informative under sk ewed class 
...
true bankruptcies as possible. This setting n aturally 
...
learning frameworks further emphasize that 
...
ratio portrait that summarizes the characteristic rati o 
...
seeds for reproducibility. Second, we report det ailed 
...
interpretable linear baseline. We used 
LogisticRegression with the liblinear solver and 
...
percentiles of th e training data (clipped to [0,1]). We 
...
class c. Intuitively, the minority class receiv es larger 
...
Attr24 gross profit (in 3 years) 
/ total assets 922 2.1 
Attr28 working capital / fixed 
assets 812 1.9 
Attr64 sales / fixed assets 812 1.9 
Attr53 equity / fixed assets 812 1.9 
Attr54 constant capital / fixed 
assets 812 1.9 
...
Attr4 current assets / short-term 
liabilities Other 
Attr7 EBIT / total assets Other 
Attr9 sales / total assets Other 
Attr10 equity / total assets Other 
Attr12 gross profit / short-term 
liabilities Liquidity/Profitability 
Attr13 (gross profit + depreciation) / 
sales Profitability (margin) 
Attr16 (gross profit + depreciation) / 
total liabilities Solvency (coverage) 
Attr24 gross profit (in 3 years) / total 
assets Profitability (long-term) 
Attr26 (net profit + depreciation) / total 
liabilities Cash flow / Leverage 
Attr27 profit on operating activities / 
financial expenses 
Coverage (operating profit / 
financial expenses) 
Attr29 logarithm of total assets Size 
Attr34 operating expenses / total 
liabilities Expense structure / Leverage 
Attr46 (current assets - inventory) / 
short-term liabilities Liquidity (quick ratio variant) 
...
LR None LogisticRegression solver=liblinear, C=1.0, 
max_iter=2000 
CostSensitive-LR class_weight=balanced LogisticRegression solver=liblinear, C=1.0, 
max_iter=2000 
ROS-LR RandomOverSampler LogisticRegression 
ROS 
sampling_strategy=auto
, random_state=42 
SMOTE-LR SMOTE LogisticRegression 
k_neighbors=5, 
sampling_strategy=auto
, random_state=42 
RUS-LR RandomUnderSampler LogisticRegression 
RUS 
sampling_strategy=auto
, random_state=42
...
default 
CostSensitive-RF class_weight=balanced
_subsample RandomForest 
n_estimators=200, 
n_jobs=1, other params 
default 
CostSensitive-RF+MI 
...
Focal-LR Focal loss Linear (PyTorch) 
alpha=0.25, 
gamma=2.0, 
epochs=300, lr=0.01, 
Adam, 
...
is consistent with financial dis tress processes: ratio 
...
severe precision collapse. SMOTE -LR improv es over 
...
portion of Figure 3 is operati onally most relevant, and 
...
function class and fixed (α,γ)=(0.25,2.0), f ocal loss 
...
as the linear baseline. This is not a marginal 
...
sensitive RF recovered 11.1% an d RF+MI recovered 
...
can enable audit teams to c oncentrate substantive 
...
with classic dis tress theory: liquidity ratios such as 
...
predictive of distress. Other important indicators 
...
coverage (lower Attr12, Attr16, A ttr46, Attr26). This 
...
concise narrativ e that is consistent with the financial 
...
indicators can meaningfully improve bankruptcy 
...
the base bankru ptcy rate. Lift captures how strongly a 
...
2ndYea
r 0.017 0.008 0.025 0.008 0.008 0.008 0.192 0.192 0.217 
3rdYea
r 0.034 0.04 0.04 0.04 0.034 0.034 0.114 0.154 0.174 
...
2ndYea
r 0.075 0.067 0.075 0.083 0.058 0.058 0.475 0.475 0.525 
3rdYea
r 0.221 0.188 0.188 0.208 0.154 0.174 0.43 0.423 0.436 
...
2ndYea
r 0.217 0.217 0.217 0.233 0.167 0.15 0.6 0.65 0.625 
3rdYea
r 0.329 0.322 0.322 0.315 0.289 0.295 0.577 0.55 0.624 
...
Vol. 4(4), pp. 80-96, April 2024  
[90] 
Audit rate (%) LR Recall CostSensitive-RF 
Recall 
CostSensitive-
RF+MI Recall 
Random Recall 
...
financial 
expenses 
0.056 1.273 1.273 0.000 
Attr24 
gross profit (in 
3 years) / total 
assets 
0.038 0.029 0.172 -0.143 
Attr26 
(net profit + 
depreciation) / 
total liabilities 
0.032 0.117 0.286 -0.169 
Attr34 
operating 
expenses / total 
liabilities 
0.028 0.926 1.738 -0.812 
Attr46 
(current assets 
- inventory) / 
short-term 
liabilities 
0.027 0.625 0.978 -0.353 
Attr16 
(gross profit + 
depreciation) / 
total liabilities 
0.027 0.125 0.315 -0.190 
Attr13 
(gross profit + 
depreciation) / 
sales 
0.026 0.038 0.082 -0.044 
Attr6 
retained 
earnings / total 
assets 
...
capital / total 
assets 
0.018 0.456 0.593 -0.137 
Attr12 
gross profit / 
short-term 
liabilities 
0.018 0.053 0.257 -0.204 
...
Missing(Attr27) profit on operating activities / 
financial expenses 0.079 
Missing(Attr11) 
...
Missing(Attr37) (current assets - inventories) / 
long-term liabilities 0.001 
...
where a model is trained once an d then applied 
...
Temporal models, survival ana lysis, or sequence 
learners could better reflect real going -concern 
assessment where auditors consider trends and 
...
horizon may change conclusions ab out its utility. 
...
Polish Companies Bankruptcy benchmark (five 
...
Across all horizons, tree ensembles substantially 
outperformed linear baselines, indicating meaningful 
non-linear structure in financial ratios. Classic 
...
firms, aligning with the intuition of c lassic distress 
...
“Ensemble boosted trees with synthetic features 
...
“Claim-Aware Scientific RAG: Evidence -First 
...
distributed systems using machine learning,” 
...
vol. 4, no. 6, pp. 48 –63, Jun. 2024, doi: 
